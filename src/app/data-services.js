...
async function syncCartofriches(){
  ...
  try{
    ...
    while(all.length<total&&page<=50){
      ...
      els.cartofrichesHelp.textContent=`Cartofriches · ${Math.min(all.length,total).toLocaleString("fr-FR")} / ${total.toLocaleString("fr-FR")} fiches départementales reçues…`;
...
function heritageCoordinatePairFromText(value){
  const textValue=normalizeLooseText(value).trim();if(!textValue)return null;
  const wkt=parseWktPoint(textValue);if(wkt)return [wkt.lat,wkt.lon];
  const patterns=[
    /^\s*(-?\d+,\d+)\s*,\s*(-?\d+,\d+)\s*$/,
    /^\s*(-?\d+(?:[.,]\d+)?)\s*[;|/]\s*(-?\d+(?:[.,]\d+)?)\s*$/,
    /^\s*(-?\d+(?:\.\d+)?)\s*,\s*(-?\d+(?:\.\d+)?)\s*$/,
  ...
  for(const pattern of patterns){
    const match=textValue.match(pattern);if(!match)continue;
    const a=Number(match[1].replace(",",".")),b=Number(match[2].replace(",","."));
...
function heritageCoordinates(record){
  ...
  const candidates=[...named,...entries.map(([,v])=>v).filter(v=>v!=null&&v!=="")];
  for(const raw of candidates){
    ...
    const nums=heritageCoordinatePairFromText(raw);
    if(nums){
...
function normalizeCultureRecord(record,category){
  ...
  const name=heritageField(f,[/titre.*courant/i,/titre.*editorial/i,/denomination.*edifice/i,/nom.*officiel/i,/^nomoff$/i,/^nom$/i,/appellation/i,/intitule/i],CULTURE_DATASETS[category]?.label||"Lieu patrimonial");
...
async function fetchJsonMaybeGzip(url,attempts=2){
  let r=null,lastError=null;
  for(let attempt=0;attempt<attempts;attempt++){
    try{
      r=await fetch(url,{mode:"cors",credentials:"omit",cache:attempt?"reload":"default"});
      if(!r.ok)throw new Error(`HTTP ${r.status}`);
      break;
    }catch(error){lastError=error;if(attempt+1<attempts)await new Promise(resolve=>setTimeout(resolve,160))}
  }
  if(!r?.ok)throw lastError||new Error("Téléchargement impossible");
...
async function fetchCadastre(){
  ...
  if(failures.length<2){
    ...
    if(els.cadastreStatus)els.cadastreStatus.title=failures.map(result=>result.reason?.message||String(result.reason)).join(" · ");
    console.warn("Cadastre indisponible",...failures.map(result=>result.reason));
  }
