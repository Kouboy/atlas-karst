...
function carnetSummary(document){
  ...
  return {
    observations:content.observations?.length||0,personalMarkers:content.personalMarkers?.length||0,undergroundHypotheses:content.undergroundHypotheses?.length||0,landscapeChanges:content.landscapeChanges?.length||0,
    notes:content.notes?.length||0,annotations:Object.keys(content.annotations||{}).length,
    embeddedRecords:(extracts.officialCavities?.length||0)+(extracts.cartofriches?.length||0)+(extracts.heritageItems?.length||0)+(extracts.bss?.length||0)+(extracts.hydrometry?.length||0)+(extracts.biodiversity?.length||0)+(extracts.landCover?.length||0)+(extracts.geology?.length||0)+(extracts.address?1:0),
...
async function buildAtlasCarnet(snapshot=buildAtlasSnapshot()){
  ...
  const document={
    ...
    content:{
      ...
      observations:carnetArray(d.observations),personalMarkers:carnetArray(d.personalMarkers),undergroundHypotheses:carnetArray(d.undergroundHypotheses),landscapeChanges:carnetArray(d.landscapeChanges),notes:carnetArray(d.loreItems),annotations:carnetJsonClone(d.poiAnnotations||{}),
