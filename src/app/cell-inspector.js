...
function territoryPortraitHtml(cell,x,y){
  const coord=gridToCoord(x,y,state.lastGrid.extent),radius=1600,nearby=(items=[])=>items.filter(item=>Number.isFinite(+item.lat)&&Number.isFinite(+item.lon)&&distanceMeters(coord,item)<=radius),lines=[];
  const covers=nearby(state.landCover);if(covers.length)lines.push(`<li><span class="cell-fact-kind">sol</span>${esc(covers.slice(0,3).map(item=>item.name).filter(Boolean).join(" · "))} <small>BD CARTO® · IGN</small></li>`);
  const nature=nearby(state.natureAreas);if(nature.length)lines.push(`<li><span class="cell-fact-kind">nature</span>${nature.length} espace${nature.length>1?"s":""} naturel${nature.length>1?"s":""} remarquable${nature.length>1?"s":""} à proximité <small>IGN / INPN</small></li>`);
  const hydro=nearby(state.hydrometry);if(hydro.length)lines.push(`<li><span class="cell-fact-kind">eau</span>${esc(hydro.slice(0,2).map(item=>item.river||item.name||"station hydrométrique").join(" · "))} <small>Hub’Eau</small></li>`);
  const bio=nearby(state.biodiversity);if(bio.length){const count=new Set(bio.flatMap(item=>(item.species||[]).map(species=>species.speciesKey))).size;lines.push(`<li><span class="cell-fact-kind">vivant</span>${count} espèce${count>1?"s":""} publiée${count>1?"s":""} dans les mailles voisines <small>GBIF</small></li>`)}
  const historical=[...nearby(state.heritageItems),...nearby(state.cartofriches),...nearby(state.landscapeChanges)];if(historical.length)lines.push(`<li><span class="cell-fact-kind">temps</span>${historical.length} repère${historical.length>1?"s":""} historique${historical.length>1?"s":""}, patrimonial${historical.length>1?"ux":""} ou personnel${historical.length>1?"s":""} <small>sources mixtes</small></li>`);
