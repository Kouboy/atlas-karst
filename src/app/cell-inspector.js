...
function terrainPhrase(cell,slope,x,y){
  const profile=localSlopeProfile(x,y),direction=profile?.direction;
  if(currentDepth()<0)return `La carte affiche ici la coupe interprétative ${depthSliceLabel()}. Elle représente un modèle de continuité entre les niveaux, et non le relevé d’une galerie ou d’un vide réellement reconnu.`;
  const cls=cell.cls||"";
  if(cls.includes("water"))return "Un cours d’eau ou une surface en eau est cartographié dans cette cellule. C’est le repère de surface le plus structurant de la sélection.";
  if(cls.includes("road"))return "Une voie traverse cette cellule. La carte décrit son tracé, sans en déduire les usages, le trafic ou les conditions de circulation.";
  if(cls.includes("building")||cls.includes("cad-building"))return "Une emprise bâtie est cartographiée ici. Son contour renseigne la présence d’une construction, pas son usage ni son état actuel.";
  if(cls.includes("forest"))return "La couverture boisée domine cette cellule. Elle constitue un contexte d’occupation du sol, sans renseigner à elle seule la végétation présente au sol.";
  ...
  if(cls.includes("field")||cls.includes("meadow"))return "La cellule correspond à un espace ouvert, cultivé ou herbacé selon les données cartographiques disponibles.";
  if(Number.isFinite(slope)){
    const tail=direction?` La pente semble s’abaisser vers le ${direction}.`:"";
    if(slope<1.5)return `Le modèle de relief indique une pente locale très faible (≈ ${slope.toFixed(1)}°).${tail}`;
    if(slope<5)return `Le modèle de relief indique une pente douce (≈ ${slope.toFixed(1)}°).${tail}`;
    if(slope<11)return `Le modèle de relief indique une pente marquée (≈ ${slope.toFixed(1)}°).${tail}`;
    return `Le modèle de relief indique une pente forte (≈ ${slope.toFixed(1)}°).${tail}`;
  }
...
function localContextItems(x,y,radius=1600){
  ...
  return {
    covers:nearby(state.landCover),geology:nearby(state.geology),nature:nearby(state.natureAreas),hydro:nearby(state.hydrometry),bio:nearby(state.biodiversity),
    heritage:nearby(state.heritageItems),industrial:nearby(state.cartofriches),observations:nearby(state.observations),personal:nearby(state.personalMarkers),changes:nearby(state.landscapeChanges)
  };
}
function localContextNarrative(x,y){
  if(currentDepth()<0)return "Les couches de surface et les documents voisins restent consultables ci-dessous : ils apportent du contexte, sans valider la forme souterraine affichée.";
  const context=localContextItems(x,y),sentences=[];
  const coverNames=[...new Set(context.covers.map(item=>item.name).filter(Boolean))].slice(0,2);
  if(coverNames.length)sentences.push(`À proximité immédiate, l’occupation du sol cartographiée est ${coverNames.join(" et ")}.`);
  const formations=[...new Set(context.geology.map(item=>item.name).filter(Boolean))].slice(0,2);
  if(formations.length)sentences.push(`À l’échelle régionale, la carte géologique du BRGM mentionne ${formations.join(" et ")}.`);
  const natural=[];
  if(context.nature.length)natural.push(`${context.nature.length} espace${context.nature.length>1?"s":""} naturel${context.nature.length>1?"s":""} inventorié${context.nature.length>1?"s":""}`);
  if(context.hydro.length)natural.push(`${context.hydro.length} station${context.hydro.length>1?"s":""} hydrométrique${context.hydro.length>1?"s":""}`);
  if(context.bio.length){const species=new Set(context.bio.flatMap(item=>(item.species||[]).map(species=>species.speciesKey))).size;if(species)natural.push(`${species} espèce${species>1?"s":""} citée${species>1?"s":""}`)}
  if(natural.length)sentences.push(`Les données de voisinage signalent ${natural.join(", ")}.`);
  const human=[];
  if(context.heritage.length)human.push(`${context.heritage.length} repère${context.heritage.length>1?"s":""} patrimonial${context.heritage.length>1?"ux":""}`);
  if(context.industrial.length)human.push(`${context.industrial.length} site${context.industrial.length>1?"s":""} d’activité recensé${context.industrial.length>1?"s":""}`);
  if(context.observations.length||context.personal.length||context.changes.length)human.push(`${context.observations.length+context.personal.length+context.changes.length} note${context.observations.length+context.personal.length+context.changes.length>1?"s":""} de carnet`);
  if(human.length)sentences.push(`Pour l’histoire et les usages du lieu, le carnet rassemble ${human.join(", ")}.`);
...
function technicalCellLines(cell,x,y){
  ...
  if(cell.feature){
    ...
    if(f.river)parts.push(`Cours d’eau / site : ${esc(f.river)}`);
    ...
    if(f.url&&(f.heritage||f.hydrometry)){const href=safeExternalUrl(f.url);if(href)parts.push(`<a href="${escAttr(href)}" target="_blank" rel="noopener noreferrer">ouvrir la notice source ↗</a>`)}
...
function territoryPortraitHtml(cell,x,y){
  const {covers,geology,nature,hydro,bio,heritage,industrial,observations,personal,changes}=localContextItems(x,y),lines=[];
  if(covers.length)lines.push(`<li><span class="cell-fact-kind">sol</span>${esc(covers.slice(0,3).map(item=>item.name).filter(Boolean).join(" · "))} <small>BD CARTO® · IGN</small></li>`);
  if(geology.length)lines.push(`<li><span class="cell-fact-kind">géologie</span>${esc(geology.slice(0,2).map(item=>item.name).filter(Boolean).join(" · "))} <small>BRGM · contexte régional</small></li>`);
  if(nature.length)lines.push(`<li><span class="cell-fact-kind">nature</span>${nature.length} espace${nature.length>1?"s":""} naturel${nature.length>1?"s":""} remarquable${nature.length>1?"s":""} à proximité <small>IGN / INPN</small></li>`);
  if(hydro.length)lines.push(`<li><span class="cell-fact-kind">eau</span>${esc(hydro.slice(0,2).map(item=>item.river||item.name||"station hydrométrique").join(" · "))} <small>Hub’Eau</small></li>`);
  if(bio.length){const count=new Set(bio.flatMap(item=>(item.species||[]).map(species=>species.speciesKey))).size;lines.push(`<li><span class="cell-fact-kind">vivant</span>${count} espèce${count>1?"s":""} publiée${count>1?"s":""} dans les mailles voisines <small>GBIF</small></li>`)}
  const historical=[...heritage,...industrial,...changes,...observations,...personal];if(historical.length)lines.push(`<li><span class="cell-fact-kind">carnet</span>${historical.length} repère${historical.length>1?"s":""} historique${historical.length>1?"s":""}, patrimonial${historical.length>1?"ux":""} ou personnel${historical.length>1?"s":""} <small>sources mixtes</small></li>`);
  if(!lines.length)return "";
  return `<section class="cell-section cell-section-portrait"><h3>Portrait du lieu</h3><ul class="cell-facts">${lines.join("")}</ul><p class="cell-source-line">Rayon de lecture ≈ 1,6 km. Cette synthèse rapproche des couches ; elle ne les fusionne pas en une nouvelle donnée.</p></section>`;
}
