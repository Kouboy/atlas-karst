...
function runAtlasSelfCheck(){
  ...
  checks.push(debugCheckResult("Gestionnaire de territoires",territoryControllerRuntime.ready&&territoryControllerRuntime.bound&&territoryControllerRuntime.managerReady&&!territoryControllerRuntime.lastError,`${territoryControllerRuntime.librarySize} enregistrés · ${territoryControllerRuntime.saves}/${territoryControllerRuntime.loads} sauvegardes/ouvertures · dernière : ${territoryControllerRuntime.lastAction}`));
  ...
  checks.push(debugCheckResult("Architecture du carnet",sidebarSections.length===4&&visibleSidebarSections.length===1&&uiShellRuntime.nativeSubsections===7,`${sidebarSections.length} sections · ${uiShellRuntime.nativeSubsections} sous-sections natives · section ${uiShellRuntime.activeSection}`));
  ...
  checks.push(debugCheckResult("Registre des sources",sourceRegistryRuntime.ready&&SOURCE_REGISTRY.length===13&&!sourceRegistryRuntime.lastError,`schéma ${SOURCE_REGISTRY_SCHEMA_VERSION} · ${SOURCE_REGISTRY.length} sources · ${sourceRegistryRuntime.catalogRenders} catalogue · ${sourceRegistryRuntime.statusUpdates} statuts`));
  checks.push(debugCheckResult("Hydrométrie bornée",hydrometryRuntime.ready&&state.hydrometry.length<=HYDROMETRY_STATION_CAP,`${state.hydrometry.length}/${HYDROMETRY_STATION_CAP} stations · ${hydrometryRuntime.measured} mesurées · ${hydrometryRuntime.observationRequests} requêtes de mesure`));
  checks.push(debugCheckResult("Biodiversité agrégée",biodiversityRuntime.ready&&state.biodiversity.length<=Math.ceil(CONFIG.dataWidthKm)*Math.ceil(CONFIG.dataHeightKm),`${biodiversityUniqueSpecies()} espèces · ${state.biodiversity.length} mailles · ${biodiversityRuntime.rawReceived} occurrences échantillonnées`));
  checks.push(debugCheckResult("Espaces naturels bornés",natureAreasRuntime.ready&&state.natureAreas.length<=NATURE_ENDPOINTS.length*NATURE_AREA_CAP,`${state.natureAreas.length}/${NATURE_ENDPOINTS.length*NATURE_AREA_CAP} zones · ${natureAreasRuntime.requests} requêtes`));
...
function createDebugReport(){
  ...
  return [
    ...
    `Gestionnaire de territoires : ${territoryControllerRuntime.managerReady?"prêt":"incomplet"} · ${territoryControllerRuntime.librarySize} enregistrés · ${territoryControllerRuntime.created} créations · ${territoryControllerRuntime.saves}/${territoryControllerRuntime.loads} sauvegardes/ouvertures · ${territoryControllerRuntime.renames}/${territoryControllerRuntime.duplicates}/${territoryControllerRuntime.deletes} renommages/duplications/suppressions · ${territoryControllerRuntime.autosaves} sauvegardes automatiques · erreur ${territoryControllerRuntime.lastError||"aucune"}`,
