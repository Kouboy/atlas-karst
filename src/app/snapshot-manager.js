...
const SNAPSHOT_LAYER_KEYS=["layerSurface","layerRelief","layerCadastreBuildings","layerParcels","layerBss","layerHydrometry","layerObservations","layerHeritage","layerLore","layerCartofriches","layerCavities","layerHypothesis","layerHydrology","layerLabels","layerHouse","ambientMotion"];
...
function buildAtlasSnapshot(){
  ...
  return {
    ...
    data:{
      ...
      bss:state.bss||[],hydrometry:state.hydrometry||[],elevation:state.elevation||null,observations:state.observations||[],loreItems:state.loreItems||[],encounterCollection:state.encounterCollection||{},encounterEnabled:!!state.encounterEnabled
    }
  };
}
...
function applyAtlasSnapshot(snapshot,{source="instantané",renderNow=true}={}){
  ...
  state.address=d.address||null;state.bss=Array.isArray(d.bss)&&d.bss.length?d.bss:mergeBssItems(territoryUsesEmbeddedData("bss",CONFIG.territory)?BSS_EMBEDDED_LOCAL:[]);state.hydrometry=Array.isArray(d.hydrometry)?d.hydrometry:[];state.elevation=d.elevation||null;
  ...
  refreshCavities();updateBssUI();updateHydrometryUI();updateCartofrichesUI();updateHeritageUI();populateCavitySelect();
